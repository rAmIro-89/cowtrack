...
def main() -> None:
    parser = argparse.ArgumentParser(description="Evaluate small-object tiled detectors vs baseline")
    ...
    parser.add_argument("--output_root", default="data/interim/tiling_smallobj_model_comparison")
    ...
    parser.add_argument("--smallobj_experiments_csv", default="reports/tiling_smallobj/tiled_smallobj_experiments.csv")
    ...
    smallobj_csv = (project_root / args.smallobj_experiments_csv).resolve()
    if not smallobj_csv.exists():
        raise FileNotFoundError(f"Smallobj experiments CSV not found: {smallobj_csv}")

    with smallobj_csv.open("r", encoding="utf-8") as f:
        rows = list(csv.DictReader(f))
    models.extend(rows)
    ...
    out_csv = reports_dir / "smallobj_vs_baseline_comparative.csv"
    ...
    if improved:
        conclusion = "Small-object tiling mejora la deteccion y conviene continuar con tracking sobre el mejor detector."
    else:
        conclusion = "Esta iteracion small-object aun no supera baseline; siguiente cuello: etiquetas y hard negative mining."

    out_md = reports_dir / "smallobj_vs_baseline_comparative.md"
    lines = [
